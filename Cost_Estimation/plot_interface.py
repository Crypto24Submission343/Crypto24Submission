...
def draw_intersecting_lines(ax, n, log_M, max_depth, leg_font_size = 11):
	...
	# TARGET SECURITY
	# ax.axhline(y=Tools.kyber_expected_security[n], dashes=[1, 1], color='#B90E0A', alpha=1, lw=1, label='Target bit security of Kyber')
	ax.axhline(y=Tools.kyber_expected_security[n], dashes=[1, 1], color='black', alpha=1, lw=1, label='Target bit security of Kyber')
	line_leg_vert_align = 'bottom'
	if n == 623 and max_depth == 96:
		z_coord_line_leg = 43
	...
	#text(z_coord_line_leg-5, Tools.kyber_expected_security[n], "Targeted bit security", horizontalalignment='left', verticalalignment=line_leg_vert_align, color='black', fontsize=leg_font_size)
...
def anotate_critical_values(ax, n, max_depth, log_M, z_to_cost, label_fontsize = 12):
	...
	y_offset = max(35, max([z_to_cost[min_z].log_gcost_total()]) / 3.2)
...
def plot_cost_estimation_z_to_g_cost(plot_dir, filename, z_to_cost, max_depth, n, log_M, circuit, prefix, postfix, anotate=True, leg_font_size = 12):
	...
	if max_depth == 0:
		...
		ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.57), fontsize=leg_font_size, framealpha=0.8)
	# elif n == 623:
	# 	#ax.legend(loc='lower left', bbox_to_anchor=(0, 0), fontsize=leg_font_size, framealpha=0.8)  # (horizontal, vertical)
	# 	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.57), fontsize=leg_font_size, framealpha=0.8)  # (horizontal, vertical)
	#
	# else:
	# 	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.57), fontsize=leg_font_size, framealpha=0.8)  # (horizontal, vertical)
