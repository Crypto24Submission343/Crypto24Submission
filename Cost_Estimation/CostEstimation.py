...
class CostEstimation:
    ...
    def tqdm_body(self, z_m_k_to_cost, log_z, step_size_z, log_M, log_M_lower, log_Y, step_size, circuit, max_depth):
        ...
        for log_m in range(log_M_lower, log_M + 1, step_size):
            ...
            self.pool_func_get_costs(proc_id, data, log_Y, max_depth, return_dict)
        ...
        if self.use_cached:
            costFiles = Tools.DataFiles(max_depth, self.n, self.boundCoefficient, self.modulus_q, self.const_reps_QPE, self.const_reps_W, self.num_nodes_branching_dfs, self.force_DF, self.bound)
