...
class TreeHeuristics:
    ...
    def log_Hk_M(self, k, n, nbases, pprime, bound="lower", verb=False):
        """
        ...
        if k == 1:
            ...
            elif bound == "lowerlower":
                x = Tools.doNumber(
                    self.lattice_class.log_Hk(k=k-1, n=n, nbases=nbases, pprime=pprime)
                )
            else:
                raise ValueError("log_Hk_M bound can only be upper or lower")
            if verb:
    ...
    def log_avg_Skh_M(self, k, h, n, nbases, pprime, verb=False):
        ...
        if self.bound == "upper":
            log_Hk_M_kph = self.log_Hk_M(k + h, n, nbases, pprime, bound="upper", verb=False)
            log_Hk_M_k = self.log_Hk_M(k, n, nbases, pprime, bound="upper", verb=False)
        elif self.bound == "lower":
            log_Hk_M_kph = self.log_Hk_M(k + h, n, nbases, pprime, bound="lower", verb=False)
            log_Hk_M_k = self.log_Hk_M(k, n, nbases, pprime, bound="upper", verb=False)
        elif self.bound == "lowerlower":
            log_Hk_M_kph = self.log_Hk_M(k + h, n, nbases, pprime, bound="lower", verb=False)
            log_Hk_M_k = self.log_Hk_M(k, n, nbases, pprime, bound="lower", verb=False)
        else:
            raise ValueError("lattice_bound can only be upper or lower")
        x = log_Hk_M_kph - log_Hk_M_k
        # x = self.log_Hk_M(k + h, n, nbases, pprime, verb=False) - self.log_Hk_M(k, n, nbases, pprime, verb=False)
        if verb:
            print(
                f" . . (log_avg_Skh_M) k {k}, h {h}, n {n}, log_n {round(math.log(nbases, 2))}, --> log_avg_Skh_M: {round(float(x), 3)} = {log_Hk_M_kph} - {log_Hk_M_k}")
