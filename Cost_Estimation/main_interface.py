...
def pool_func(cost_classical_quantum, key, md, circuit, log_M, log_Y, step_size, log_Z, step_size_Z, log_M_lower, return_dict):
    return_dict[(circuit.desc,md,key)] = cost_classical_quantum.estimate_quantum_enumeration(md, circuit, log_M, log_Y, step_size, log_Z, step_size_Z, log_M_lower)

def loop_cost_estimation(kyber_sets, lst_md, circs,
                         log_M, log_Y, step_size, log_Z, step_size_Z, log_M_lower,
                         num_cores, use_cached,
                         prefix, postfix,
                         boundCoefficient, const_reps_QPE, const_reps_W, num_nodes_branching_dfs, force_DF, bound="upper"):
    """
    ...
    print(f"CostEstimation uses bound={bound}")
    print(f"Cache values:", use_cached)
    ...
    for circ in circs:
        for md in lst_md:
            for key in kyber_sets:
                ...
                cost_classical_quantum = CostEstimation.CostEstimation(n=Kyber[key]['beta'], q=Kyber[key]['q'], boundCoefficient=boundCoefficient, bound=bound,
                ...
                pool_inputs.append([cost_classical_quantum, key, md, circuit, log_M, log_Y, step_size, log_Z, step_size_Z, log_M_lower, return_dict])
            # End Kyber
        # End MaxDepths

    # POOL IT
    pool_size = max(1, min(num_cores, 0.9 * mpp.cpu_count() - 1))
    print(f"Using: {pool_size} cores")

    print(f"pool ins {len(pool_inputs)}")
    ...
    if tqdm_present:
        for _ in tqdm.tqdm(pool.istarmap(pool_func, pool_inputs), total=len(pool_inputs)):
            pass
    else:
        results = pool.starmap(pool_func, pool_inputs)
    ...
    for circ in circs:
        for md in lst_md:
            for key in kyber_sets:
                ...
                z_to_cheapest = return_dict[(circuit.desc,md,key)]
                resultsFiles = Tools.ResultFiles(boundCoefficient, Kyber[key]['q'], const_reps_QPE, const_reps_W, num_nodes_branching_dfs, force_DF, bound=bound)
                ...
                plot_interface.plot_cost_estimation_z_to_g_cost(resultsFiles.prepare_plot_dir(Kyber[key]['beta'], circuit),
                                                                resultsFiles.get_plot_filename(circuit, Kyber[key]['beta'], md, prefix, postfix), z_to_cheapest, md, Kyber[key]['beta'], log_M,
                                                                circuit, prefix, postfix=postfix)
            # End Kyber
            print('-' * 10)
        # End MaxDepths
        print('=' * 20)
...
def lower_bound(kyber_sets, lst_md, circuits, log_M=64, log_Y=64, step_size=1, log_Z=64, step_size_z=1, num_cores=4, use_cached=True, log_M_lower=0, bound="upper"):
    """
    ...
    loop_cost_estimation(kyber_sets, lst_md, circuits,
                         log_M, log_Y, step_size, log_Z, step_size_z, log_M_lower=log_M_lower,
                         num_cores=num_cores, use_cached=use_cached,
                         prefix=prefix, postfix='',
                         boundCoefficient=boundCoefficient, const_reps_QPE=const_reps_QPE, const_reps_W=const_reps_W, num_nodes_branching_dfs=num_nodes_branching_dfs, force_DF=force_DF, bound=bound)


def non_lower_bound(kyber_sets, lst_md, circuits, log_M, log_Y, step_size, log_Z, step_size_z, num_cores, use_cached, log_M_lower=0, bound="upper"):
    """
...
def main():
    ...
    parser.add_argument('-log-m-lower', required=False, type=int, default=64, help="Log of the minimal number of bases considered for extreme pruning")
    parser.add_argument('-bound', required=False, type=str, default="lower", help="DEBUG: uses upper/lower bound for subtree sizes")
    ...
    if args.num_cores == 0:
        num_cores = int(0.9 * multiprocessing.cpu_count()) - 1

    print(f"Quantum Enumeration Cost Estimation")
    print(f"    using up to {num_cores} cores")
    ...
    lower_bound(kyber_sets=kyber_sets, lst_md=lst_md, circuits=circuits, log_M=args.m, log_Y=args.y, step_size=args.stepsize, log_Z=args.z, step_size_z=args.stepsize_z,
                        num_cores=args.num_cores, use_cached=args.cache, log_M_lower=args.log_m_lower, bound=args.bound)
