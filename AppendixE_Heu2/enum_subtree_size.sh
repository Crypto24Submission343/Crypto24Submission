...
then
    ...
    sage plot_subtree_predictions.py -n ${N} -q ${Q} -sd ${SD} -m ${M} -beta ${BETA} -tours ${TOURS} -ghfactor ${ENUMGHBOUND} -tree-stats-filename ${STATSFN} -subtree-root-level ${SUBTREE_ROOT_LEVEL} -subtree-stats-filename ${SUBTREE_STATS_FILE} -plot-dir ${SUBTREE_PLOTDIR} -tour 14
    ...
    sage plot_children_predictions.py -n ${N} -q ${Q} -sd ${SD} -m ${M} -beta ${BETA} -tours ${TOURS} -ghfactor ${ENUMGHBOUND} -tree-stats-filename ${STATSFN} -plot-dir ${PLOTDIR} -tour 14
