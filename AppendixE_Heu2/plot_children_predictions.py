...
def gen_plots(data, plots_dir, lwe, bkz, _tour=None, _block=None, dpi=150):
    ...
    for tour_stats in data_tuple:
        ...
        for index in tour_stats_tuple:
            ...
            _index = int(index.split('-')[0])
            sim_block_prof = sim.profile[int(_index):min(int(_index)+bkz['beta'], len(sim.profile))]
            sim_gh = profile_gh(sim_block_prof)
            sim_R = sqrt(bkz['ghfactor']) * sim_gh
            stats = tour_stats[index]

            stats_keys = set(stats.keys())
            gs2 = stats['gs'][int(_index):min(int(_index)+bkz['beta'], len(sim.profile))]
            ...
            theory_ln_upup = [ (int(x)+1, int(int(x) == 0) + (0.5 if int(x) == 0 else 1) * 2 * sim_R / sqrt(sim.profile[min(int(_index)+bkz['beta'], len(sim.profile))-int(x)-1]) ) for x in stats_keys ] # if k \approx 1, add a (1 if int(x) == 0 else 2) factor
